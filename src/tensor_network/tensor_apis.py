...
class NumpyAPILog(NumpyAPI):
    def __init__(self, entry_type, thread_limit=None):
        super().__init__(entry_type, thread_limit)
        self.lognorm = 0

    def get_entry(self, tup):
        return self.lognorm + self._numpy.log(self[tup])
    ...
    def normalize(self, a):
        norm = self.numpy.amax(a.nparray)
        if norm !=0:
            self.nparray /= norm
            self.lognorm += self.numpy.log(norm)
